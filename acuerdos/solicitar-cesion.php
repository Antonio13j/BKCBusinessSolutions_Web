...
if(in_array($doc['estado'],['PENDIENTE','ERROR'],true)){$db->prepare("UPDATE documentos SET estado='PENDIENTE',solicitado_por=?,updated_at=NOW() WHERE id=?")->execute([$u['id'],$id]);audit($db,$u['id'],$u['username'],'DOCUMENT_REQUESTED','DOCUMENTO',$id,null,['tipo'=>'CESION_DERECHOS']);}
